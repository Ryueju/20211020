...
public class Game extends Thread { // thread형태로 구현하기위해 thread상속받아줌
	...
	private Image player = new ImageIcon("src/main/java/minicat.png").getImage();
	...
	private void bonusGetProcess() {
		...
		for (int i = 0; i< bonusList.size(); i++) {
			...
			bonus.shoot();
			if(bonus.x > playerX & bonus.x < playerX + playerWidth && bonus.y >  playerY && bonus.y < playerY + playerHeight) {
				score += bonus.score;
